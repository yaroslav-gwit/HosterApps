...
# Shared post-install helpers for LeilFS host-side installers. The callers are
# expected to source this file after enabling "set -euo pipefail".

: "${SCRIPT_NAME:=leilfs-installer}"
...
: "${LEILFS_INSTALL_RUNTIME_DEPS:=${SAUNAFS_INSTALL_RUNTIME_DEPS:-1}}"
: "${LIMITS_CONF:=/etc/security/limits.d/10-leilfs.conf}"
: "${PAM_CONF:=/etc/pam.d/saunafs}"
: "${CGISERV_DEFAULTS_FILE:=/etc/sysconfig/saunafs-cgiserv}"
: "${LEGACY_CGISERV_DEFAULTS_FILE:=/etc/default/saunafs-cgiserv}"
...
: "${LEIL_CGI_COMPAT_PATH:=/usr/share/sfscgi}"
: "${LEIL_CGI_BACKUP_PATH:=/usr/share/sfscgi.saunafs-pre-leilfs}"
...
collect_missing_shared_libraries() {
	...
	local bundled_library_path="/usr/lib/leilfs/bundled"
	...
	if [[ -d /usr/lib/leilfs/libexec ]]; then
		mapfile -t binaries < <(find /usr/lib/leilfs/libexec -maxdepth 1 -type f -print)
	fi
	...
	for binary in "${binaries[@]}"; do
		...
		while IFS= read -r missing_library; do
			...
			LD_LIBRARY_PATH="${bundled_library_path}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}" \
...
ensure_runtime_dependencies() {
	...
	if [[ "${LEILFS_INSTALL_RUNTIME_DEPS}" != "1" ]]; then
		die "Install the required runtime libraries and rerun the installer, or leave LEILFS_INSTALL_RUNTIME_DEPS=1 so it can resolve them automatically."
	fi
...
ensure_dashboard_runtime_dependencies() {
	...
	local python_bundle_dir="/usr/lib/leilfs/python"

	[[ -x /usr/sbin/leil-cgiserver ]] || return

	if ! command -v python3 >/dev/null 2>&1; then
		if [[ "${LEILFS_INSTALL_RUNTIME_DEPS}" != "1" ]]; then
			die "The CGI dashboard requires Python 3. Install it and rerun the installer, or leave LEILFS_INSTALL_RUNTIME_DEPS=1 so it can be installed automatically."
		fi

		if is_debian_like_host; then
			require_command apt-get
			info "Installing the Python 3 runtime required by the CGI dashboard"
			DEBIAN_FRONTEND=noninteractive apt-get update
			DEBIAN_FRONTEND=noninteractive apt-get install --yes --no-install-recommends \
				"${python_package}"
		elif is_rhel_like_host; then
			require_command dnf
			info "Installing the Python 3 runtime required by the CGI dashboard"
			dnf install --assumeyes "${python_package}"
		else
			die "The CGI dashboard requires Python 3, and automatic installation is only supported on Debian/Ubuntu and RHEL/Rocky/Fedora hosts."
		fi
	fi

	if ! PYTHONPATH="${python_bundle_dir}" python3 -c 'import chardet' >/dev/null 2>&1; then
		die "The bundled Python chardet module required by the CGI dashboard is missing or unusable"
	fi
}
...
seed_default_configs() {
	if [[ ! -e "${SAUNAFS_ETC_DIR}/leil-master.cfg" ]]; then
		copy_if_missing "${MASTER_EXAMPLES_DIR}/leil-master.cfg" "${SAUNAFS_ETC_DIR}/leil-master.cfg"
		cat >> "${SAUNAFS_ETC_DIR}/leil-master.cfg" <<'EOF'

# Installed by the LeilFS helper to make the default role explicit.
PERSONALITY = master
EOF
	fi

	if [[ ! -e "${SAUNAFS_ETC_DIR}/leil-chunkserver.cfg" ]]; then
		copy_if_missing "${CHUNKSERVER_EXAMPLES_DIR}/leil-chunkserver.cfg" "${SAUNAFS_ETC_DIR}/leil-chunkserver.cfg"
		cat >> "${SAUNAFS_ETC_DIR}/leil-chunkserver.cfg" <<'EOF'

# Installed by the LeilFS helper for a simple single-host deployment.
MASTER_HOST = 127.0.0.1
EOF
	fi

	if [[ ! -e "${SAUNAFS_ETC_DIR}/leil-metalogger.cfg" ]]; then
		copy_if_missing "${METALOGGER_EXAMPLES_DIR}/leil-metalogger.cfg" "${SAUNAFS_ETC_DIR}/leil-metalogger.cfg"
		cat >> "${SAUNAFS_ETC_DIR}/leil-metalogger.cfg" <<'EOF'
...
EOF
	...
	if [[ ! -e "${SAUNAFS_ETC_DIR}/leil-mount.cfg" ]]; then
		cat > "${SAUNAFS_ETC_DIR}/leil-mount.cfg" <<'EOF'
# Default client-side mount settings for a local single-node deployment.
sfsmaster=127.0.0.1
sfsport=9421
EOF
		chmod 0644 "${SAUNAFS_ETC_DIR}/leil-mount.cfg"
	fi

	if [[ ! -e "${SAUNAFS_ETC_DIR}/leil-hdd.cfg" ]]; then
		cat > "${SAUNAFS_ETC_DIR}/leil-hdd.cfg" <<EOF
# Default single-node chunk storage created by the LeilFS helper.
# Replace this path with dedicated storage before using the host in production.
${SAUNAFS_CHUNK_DIR}
EOF
		chmod 0644 "${SAUNAFS_ETC_DIR}/leil-hdd.cfg"
	fi

	if [[ ! -e "${SAUNAFS_ETC_DIR}/leil-exports.cfg" ]]; then
		cat > "${SAUNAFS_ETC_DIR}/leil-exports.cfg" <<'EOF'
...
127.0.0.1               .       rw
EOF
		chmod 0644 "${SAUNAFS_ETC_DIR}/leil-exports.cfg"
	fi

	copy_if_missing "${MASTER_EXAMPLES_DIR}/leil-goals.cfg" "${SAUNAFS_ETC_DIR}/leil-goals.cfg"
	copy_if_missing "${MASTER_EXAMPLES_DIR}/leil-topology.cfg" "${SAUNAFS_ETC_DIR}/leil-topology.cfg"
	copy_if_missing "${MASTER_EXAMPLES_DIR}/leil-globaliolimits.cfg" "${SAUNAFS_ETC_DIR}/leil-globaliolimits.cfg"
	copy_if_missing "${CLIENT_EXAMPLES_DIR}/leil-iolimits.cfg" "${SAUNAFS_ETC_DIR}/leil-iolimits.cfg"
	copy_if_missing "${CLIENT_EXAMPLES_DIR}/leil-tls.cfg" "${SAUNAFS_ETC_DIR}/leil-tls.cfg"

	if [[ -f "${URAFT_EXAMPLES_DIR}/leil-uraft.cfg" ]]; then
		copy_if_missing "${URAFT_EXAMPLES_DIR}/leil-uraft.cfg" "${SAUNAFS_ETC_DIR}/leil-uraft.cfg"
	fi

	if [[ -f "${LEGACY_CGISERV_DEFAULTS_FILE}" && ! -e "${CGISERV_DEFAULTS_FILE}" ]]; then
		cp -p "${LEGACY_CGISERV_DEFAULTS_FILE}" "${CGISERV_DEFAULTS_FILE}"
		note "Migrated CGI service overrides to ${CGISERV_DEFAULTS_FILE}"
	fi
...
# Promote SaunaFS configuration filenames to their canonical LeilFS names.
# Existing LeilFS files always win, and the legacy files are retained so the
# host can be inspected or rolled back without reconstructing configuration.
migrate_legacy_configs() {
	...
	for pair in "${config_pairs[@]}"; do
		legacy_name="${pair%%:*}"
		leil_name="${pair#*:}"
		if [[ -f "${SAUNAFS_ETC_DIR}/${legacy_name}" && ! -e "${SAUNAFS_ETC_DIR}/${leil_name}" ]]; then
			cp -p "${SAUNAFS_ETC_DIR}/${legacy_name}" "${SAUNAFS_ETC_DIR}/${leil_name}"
			note "Migrated ${SAUNAFS_ETC_DIR}/${legacy_name} to ${leil_name}"
			((migrated += 1))
		fi
	done

	if (( migrated > 0 )); then
		info "Migrated ${migrated} SaunaFS configuration files; legacy copies were preserved"
	fi
}

# LeilFS replaces SaunaFS's real sfscgi directory with a compatibility symlink
# to leil-cgi.  GNU tar and cmake --install cannot overlay a symlink onto a
# non-empty directory. Preserve that directory before installing the new tree,
# including any local CGI customizations an operator may have made.
prepare_legacy_path_transitions() {
	local backup_path="${LEIL_CGI_BACKUP_PATH}"
	local backup_suffix=0

	if [[ ! -d "${LEIL_CGI_COMPAT_PATH}" || -L "${LEIL_CGI_COMPAT_PATH}" ]]; then
		return
	fi

	while [[ -e "${backup_path}" || -L "${backup_path}" ]]; do
		((backup_suffix += 1))
		backup_path="${LEIL_CGI_BACKUP_PATH}.${backup_suffix}"
	done

	note "Preserving legacy CGI directory as ${backup_path}"
	mv -- "${LEIL_CGI_COMPAT_PATH}" "${backup_path}"
}
...
# LeilFS systemd units that are currently in the "active" state.
...
stop_active_services() {
	...
	warn "Stopping running LeilFS services before upgrade:"
	local unit=""
...
restart_previously_active_services() {
	...
	info "Restarting LeilFS services that were active before the upgrade:"
	local unit=""
	for unit in "${SAUNAFS_ACTIVE_SERVICES[@]}"; do
		print_value "${unit}"
...
refresh_dynamic_linker_cache() {
	...
	# versions.  Bundled libraries are now kept private to LeilFS processes via
	# LD_LIBRARY_PATH wrapper scripts in /usr/lib/leilfs/libexec/.
...
print_next_steps() {
	printf '\n'
	info "LeilFS was installed into the packaged layout."
	printf '\n'

	note "Review these files before starting services:"
	print_value "${SAUNAFS_ETC_DIR}/leil-master.cfg"
	print_value "${SAUNAFS_ETC_DIR}/leil-chunkserver.cfg"
	print_value "${SAUNAFS_ETC_DIR}/leil-metalogger.cfg"
	print_value "${SAUNAFS_ETC_DIR}/leil-hdd.cfg"
	print_value "${SAUNAFS_ETC_DIR}/leil-exports.cfg"
	printf '\n'
