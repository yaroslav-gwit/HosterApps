...
# This helper installs LeilFS from an already-prepared CMake build tree.
# It does not compile anything on the host. The intended flow is:
#   1. build LeilFS in Docker
...
# LeilFS binaries embed default config/data paths such as /etc/saunafs and
...
note "Installing LeilFS from the existing build tree in ${BUILD_DIR}"
